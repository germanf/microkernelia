...
## Prompting Strategy (ES-KNN → USC)
**Language**: English.  
**Reference**: See [`.github/prompts/AIO.prompt.md`](./prompts/AIO.prompt.md) for detailed phasing, techniques, and model recommendations.

### Phase 1 — Task Definition (TaskDefinitionRequest)
Request precise task definition:
- Context & objective: What exactly do you want to achieve?
- Environment & language: Tools, targets, constraints (e.g., `no_std`).
- Expected inputs/outputs: Types, formats, paths.
- Success criteria: Validation, tests, artifacts.

### Phase 2 — Deep Understanding (UnderstandingPhase · ES-KNN)
Build understanding via exemplar selection by similarity:
...
Organize context and execute with consistency:
- Structure solution: steps, decisions, dependencies.
- Implement, validate, produce minimal deliverables.
- Self-refine if inconsistencies detected.

### Complementary Techniques
- **Step-Back Prompting**: Summarize general concepts before details.
- **Self-Ask**: Formulate and answer clarifying questions.
- **Tree-of-Thought**: Compare options before deciding.
- **Few-Shot Contrastive CoT**: Use correct/incorrect examples.
- **Self-Refine**: Iteratively improve output.

### Model Recommendations
- **GPT-4o**: UnderstandingPhase, ES-KNN, architectural review.
- **GPT-5 (core)**: ExecutionPhase, USC, Tree-of-Thought, Self-Refine.
- **GPT-5.1 / 5.2**: Production daily driver (APIs, services, logic).
- **GPT-5-Codex**: Codegen, tests, guided refactors.
- **Claude Opus 4.5**: Architecture, long reasoning.
- **Claude Sonnet 4 / 4.5**: Clean code + explanations.
- **Raptor mini / Grok**: Fast ideation; avoid critical logic.
